...
class CoarseGenerator(nn.Module):
    def __init__(self, input_dim, cnum, use_cuda=True, device_ids=None):
        ...
        self.conv2_upsample = nn.UpsamplingBilinear2d(size=(128,128))
...
class ContextualAttention(nn.Module):
    ...
    def forward(self, f, b, mask=None):
        ...
        if mask is None:
            ...
            mask = F.interpolate(mask, size=(int_bs[2], int_bs[3]), mode='nearest')
        ...
        if self.rate != 1:
            # flow = F.interpolate(flow, size=self.rate*4, mode='nearest')
            flow = F.interpolate(flow, size=112, mode='nearest')
...
class LocalDis(nn.Module):
    def __init__(self, config, use_cuda=True, device_ids=None):
        ...
        self.linear = nn.Linear(self.cnum*4*4*4, 1)
...
class GlobalDis(nn.Module):
    def __init__(self, config, use_cuda=True, device_ids=None):
        ...
        self.conv1_upsample = nn.UpsamplingBilinear2d(size=(128,128))
...
if __name__ == "__main__":
    ...
    mask = torch.rand((1, 1, 112, 112))
    # coarse_generator = CoarseGenerator(3, 32, False, [])
    #
    # netout = coarse_generator(x, mask)

    fine_generator = FineGenerator(3, 32, False, [])
    netout = fine_generator(x,x, mask)
    print(len(netout))
    print(netout[1].size())

    # ld = LocalDis(config={'input_dim':3,'ndf':64}, use_cuda=False, device_ids=None)
    # gd = GlobalDis(config={'input_dim':3,'ndf':64}, use_cuda=False, device_ids=None)
    # netout = ld(x)
    # print(netout.size())
    #256:
# 0 torch.Size([1, 5, 256, 256])
# 1 torch.Size([1, 32, 256, 256])
# 2 torch.Size([1, 32, 128, 128])
# 1 torch.Size([1, 64, 128, 128])
# 1 torch.Size([1, 64, 64, 64])
# 1 torch.Size([1, 128, 64, 64])
# 1 torch.Size([1, 128, 64, 64])
# C:\Users\chs\.conda\envs\pt17\lib\site-packages\torch\nn\functional.py:3103: UserWarning: The default behavior for interpolate/upsample with float scale_factor changed in 1.6.0 to align with other frameworks/libraries, and now uses scale_factor directly, instead of relying on the computed output size. If you wish to restore the old behavior, please set recompute_scale_factor=True. See the documentation of nn.Upsample for details.
#   warnings.warn("The default behavior for interpolate/upsample with float scale_factor changed "
# 11 torch.Size([1, 128, 64, 64])
# 12 torch.Size([1, 128, 128, 128])
# 13 torch.Size([1, 64, 128, 128])
# 14 torch.Size([1, 64, 256, 256])
# 15 torch.Size([1, 32, 256, 256])
# 16 torch.Size([1, 16, 256, 256])
# 17 torch.Size([1, 3, 256, 256])